...
export default function MedicalLossRatio() {
  const kpi = useSnowflakeQuery(
    'mlr-kpi',
    `SELECT
      ROUND(AVG(MLR_PCT)*100,1) AS MLR,
      ...
      ROUND(AVG(ADMIN_RATIO_PCT)*100,1) AS ADMIN
  ...
  const byLob = useSnowflakeQuery(
    ...
    `SELECT LOB_NAME,
      ...
      CASE WHEN AVG(MLR_PCT) >= MAX(MLR_THRESHOLD) THEN TRUE ELSE FALSE END AS IS_COMPLIANT
    FROM PAYER360_CUR.FINANCIAL.MART_MEDICAL_LOSS_RATIO
